...
@dataclass
class NerfThisDataParserConfig(NerfstudioDataParserConfig):
    """Blender dataset parser config"""

    _target: Type = field(default_factory=lambda: NerfThisDataParser)
    double: bool = True


@dataclass
class NerfThisDataParser(Nerfstudio):
    config: NerfThisDataParserConfig

    def __init__(self, config: NerfThisDataParserConfig):
    ...
    def _generate_dataparser_outputs(self, split="train"):
        ...
        if self.config.double:
            ...
            if len(mask_filenames) != 0:
                mask_filenames = mask_filenames + mask_filenames
            if len(depth_filenames) != 0:
                depth_filenames = depth_filenames + depth_filenames
